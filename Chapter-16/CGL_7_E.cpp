...
int main() {
    ...
    pair<Point, Point> ans = getCircleCrossPoints(c1, c2);
    
    Point ansA = ans.second, ansB = ans.first;
    if (ans.first.x < ans.second.x || (isEqual(ans.first.x, ans.second.x) && ans.second.y < ans.second.y)) {
        ansA = ans.first;
        ansB = ans.second;
    } 
